...
import { useMessage } from '@/context/MessageContext'; // メッセージコンテキストをインポート

export default function CreateRandomSetlist({ isOpen, onClose }) {
    const { currentUser } = useContext(AuthContext);
    const { setMessageInfo } = useMessage(); // メッセージコンテキストのフックを使用
    ...
    const [existingSetlists, setExistingSetlists] = useState([]);
    ...
    const fetchExistingSetlists = async () => {
        const setlistsRef = collection(db, `users/${currentUser.uid}/Setlists`);
        const setlistsSnapshot = await getDocs(setlistsRef);
        const setlists = setlistsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        setlists.sort((a, b) => b.createdAt.toDate() - a.createdAt.toDate()); // 作成日時順に並べ替え
        setExistingSetlists(setlists);
    };

    const handleAddToExistingSetlist = async () => {
        await fetchExistingSetlists();
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose}>
            <div className="flex flex-col gap-2 min-w-[600px]">
                ...
                <button onClick={handleAddToExistingSetlist} className="mt-2 bg-gray-300 hover:bg-gray-400 text-black font-bold py-2 px-4 rounded">
                    既存のセットリストに追加する
                </button>
            </div>
            {isSetlistModalOpen && (
